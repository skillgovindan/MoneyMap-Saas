...
const LentIcon = ({ size = 24, style = { marginRight: '8px', verticalAlign: 'bottom', color: '#4f46e5' } }) => (
...
const LentMoneyForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (isEditMode) {
          const record = await lentMoneyService.getLentMoneyById(id);
          if (record) {
            setFormData({
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        await lentMoneyService.patchLentMoneyById(id, dataToSave);
      } else {
        await lentMoneyService.createLentMoney(dataToSave);
      }
      navigate('/due-tracker');
    } catch (err) {
      setValidationError(err.message || 'Failed to save lent money record.');
    }
  };
    ...
    <div className="page-container form-page">
      <div className="page-header" style={{flexDirection: 'column', alignItems: 'flex-start'}}>
        <h1 className="page-title" style={{marginBottom: '0.25rem'}}>
          <LentIcon />
          {isEditMode ? 'Edit Lent Money' : 'Add Lent Money'}
        </h1>
        <p style={{ color: '#6b7280', fontSize: '0.9rem', margin: 0 }}>
          {isEditMode ? 'Update details of money you lent to someone.' : 'Record a new amount of money you gave to someone.'}
        </p>
      </div>
      ...
      <div className="form-card">
        ...
        <form onSubmit={handleSave}>
          ...
          <div className="form-actions">
            ...
            <button type="submit" className="btn btn-primary">Save Lent Money</button>
          </div>
        </form>
      </div>
